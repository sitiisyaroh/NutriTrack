...
class SetGoalsViewModel(private val repository: UserRepository) : ViewModel() {
    ...
    fun setGoals(userId: String, gender: String, dateOfBirth: String, height: Int, weight: Int, goalWeight: Int): LiveData<ResultState<SetGoalsResponse>> {
        _setGoalsResult.value = ResultState.Loading

        viewModelScope.launch {
            try {
                val response = repository.setGoals(userId, gender, dateOfBirth, height, weight, goalWeight)
